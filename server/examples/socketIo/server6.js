...
var connectSuc = function () {
    ...
    this.on('get sockets of room', function (roomName,func) {
        ...
        var ss = gf.objToStrMap(io.in(roomName).sockets)
        console.log(`get sockets of room  ${roomName}`)
        const sockets = new Map(
            [...ss].map(([k, v]) => [k, v.name])
              );
        // console.log(sockets)
        //直接传map值消失，只能转为数组传输，前端再转为map
    ...
    this.on('join room', function (roomName, func) {
        ...
        this.join(roomName, () => {
            ...
            //空间->发送给指定房间的所有socket
            //io.to   //error
            io.to(roomName).emit("message", 'a player join to room ' + roomName); // broadcast to everyone in the room
        });
    });
}
